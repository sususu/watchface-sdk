...
typedef SWIFT_ENUM(NSInteger, DPBleCoreManagerState, open) {
  DPBleCoreManagerStateUnknown = 0,
  DPBleCoreManagerStateResetting = 1,
  DPBleCoreManagerStateUnsupported = 2,
  DPBleCoreManagerStateUnauthorized = 3,
  DPBleCoreManagerStatePoweredOff = 4,
  DPBleCoreManagerStatePoweredOn = 5,
};

typedef SWIFT_ENUM(NSInteger, DialPlateLogLevel, open) {
  DialPlateLogLevelInfo = 0,
  DialPlateLogLevelWarn = 1,
  DialPlateLogLevelDebug = 2,
  DialPlateLogLevelError = 3,
};
...
///
/// \param type 文件类型。0-表盘，1-多语言，2-背景图，3-自定义，4-音乐,5-JS,8-4g模块。其它类型查阅文档。
///
/// \param zipPath zip格式的升级文件位于本地的url
///
/// \param maxFileSliceLength 文件切片长度，默认1024字节。数值越大速度越快，但可能造成设备无法响应。该值应视具体设备而定。
///
/// \param withByteAlign 是否对文件进行CRC和字节对齐处理，需要依据具体的设备支持情况来决定该参数的取值。默认false。PS：当type为3时，总是会进行CRC和对齐操作，与该参数取值无关。当type=8 时，该参数无效，不会添加CRC和对齐
///
...
///
/// \endcode} ble_watchface_status_id_t;
@property (nonatomic, strong) NSNumber * _Nullable devErrorCode;
/// 错误码
@property (nonatomic, readonly, copy) NSString * _Nonnull description;
- (nonnull instancetype)init OBJC_DESIGNATED_INITIALIZER;
- (NSString * _Nonnull)devErrorMsg SWIFT_WARN_UNUSED_RESULT;
@end
...
/// 日志委托外部处理
@property (nonatomic, weak) id <SFLogManagerDelegate> _Nullable delegate;
- (nonnull instancetype)init SWIFT_UNAVAILABLE;
+ (nonnull instancetype)new SWIFT_UNAVAILABLE_MSG("-init is unavailable");
@end


SWIFT_PROTOCOL("_TtP14SFDialPlateSDK20SFLogManagerDelegate_")
@protocol SFLogManagerDelegate <NSObject>
- (void)sfLogManagerWithManager:(SFLogManager * _Nonnull)manager onLog:(SFCommonLogModel * _Null_unspecified)log logLevel:(enum DialPlateLogLevel)level;
@end
